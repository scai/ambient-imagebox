...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "ArrayBuffer|Blob|CanvasRenderingContext|CanvasRenderingContext2D|DOMError|File|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList|SVGAnimatedString"
  },
  JSBool: {
    "^": "Interceptor;",
    toString$0: function(receiver) {
...
["", "ambient.dart", , F, {
  "^": "",
  main: [function() {
    H.IterableMixinWorkaround_forEach(["images/cat_bw.jpg", "images/milky_way.jpg", "images/mochi.jpg", "images/shanghai.jpg", "images/tulips.jpg"], new F.main_closure(document.querySelector("#images")));
  }, "call$0", "main$closure", 0, 0, 0],
  onImageLoaded: [function($event) {
    var start, img, e, t1, t2, context, imageData, r, g, b, a, row, t3, indexOfFirstPixelInRow, col, indexOfFirstByteInPixel, t4, totalPixels, alpha, logEntry;
    start = new P.DateTime(Date.now(), false);
    start.DateTime$_now$0();
    img = J.get$target$x($event);
    e = document.createElement("canvas", null);
    t1 = J.getInterceptor$x(img);
    t2 = J.getInterceptor$x(e);
    t2.set$width(e, t1.get$width(img));
    t2.set$height(e, t1.get$height(img));
    context = t2.get$context2D(e);
    context.drawImage(img, 0, 0);
    imageData = P.convertNativeToDart_ImageData(context.getImageData(0, 0, t1.get$width(img), t1.get$height(img)));
    t2 = J.getInterceptor$x(imageData);
    r = 0;
    g = 0;
    b = 0;
    a = 0;
    ...
    while (true) {
      ...
      if (typeof t3 !== "number")
        return H.iae(t3);
      indexOfFirstPixelInRow = 4 * t3 * row;
      col = 0;
      while (true) {
        t3 = t1.get$width(img);
        if (typeof t3 !== "number")
        ...
        if (!(col < t3))
          break;
        indexOfFirstByteInPixel = indexOfFirstPixelInRow + col * 4;
        t3 = t2.get$data(imageData);
        if (indexOfFirstByteInPixel >>> 0 !== indexOfFirstByteInPixel || indexOfFirstByteInPixel >= t3.length)
          return H.ioore(t3, indexOfFirstByteInPixel);
        r += t3[indexOfFirstByteInPixel];
        t3 = t2.get$data(imageData);
        t4 = indexOfFirstByteInPixel + 1;
        if (t4 >= t3.length)
          return H.ioore(t3, t4);
        g += t3[t4];
        t4 = t2.get$data(imageData);
        t3 = indexOfFirstByteInPixel + 2;
        if (t3 >= t4.length)
          return H.ioore(t4, t3);
        b += t4[t3];
        t3 = t2.get$data(imageData);
        t4 = indexOfFirstByteInPixel + 3;
        if (t4 >= t3.length)
          return H.ioore(t3, t4);
        a += t3[t4];
        ++col;
    ...
    totalPixels = C.JSInt_methods._tdivFast$1(t2.get$data(imageData).length, 4);
    r = C.JSInt_methods.$tdiv(r, totalPixels);
    g = C.JSInt_methods.$tdiv(g, totalPixels);
    b = C.JSInt_methods.$tdiv(b, totalPixels);
    alpha = C.JSInt_methods.$tdiv(a, totalPixels) / 255;
    J.set$borderColor$x(t1.get$style(img), "rgba(" + r + ", " + g + ", " + b + ", " + alpha + ")");
    t1.set$title(img, "Ambient color is rgba(" + r + ", " + g + ", " + b + ", " + alpha + ")");
    t2 = Date.now();
    new P.DateTime(t2, false).DateTime$_now$0();
    logEntry = document.createElement("li", null);
    logEntry.textContent = "Done in " + H.S(C.JSNumber_methods._tdivFast$1(P.Duration$(0, 0, 0, t2 - start.millisecondsSinceEpoch, 0, 0)._duration, 1000)) + " ms. " + H.S(t1.get$src(img));
    document.querySelector("#log").appendChild(logEntry);
  }, "call$1", "onImageLoaded$closure", 2, 0, 1],
  main_closure: {
    "^": "Closure:11;imagesContainer_0",
    call$1: function(url) {
      var e, t1;
      ...
      t1.set$src(e, url);
      t1 = t1.get$onLoad(e);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(F.onImageLoaded$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      return this.imagesContainer_0.appendChild(e);
    }
  }
},
1],
...
function dart_precompiled($collectedClasses) {
  ...
  function main_closure(imagesContainer_0) {
    this.imagesContainer_0 = imagesContainer_0;
  }
